...
export interface CreneauExistant extends CreneauHoraire {
  id: string
  cours_id: string
...
function normaliser(saisi: CreneauSaisi): CreneauHoraire | null {
 ...
 * (b) contre les créneaux de **tous les autres** cours du propriétaire.
 *
...
export function detecterConflitsFormulaire(
  ...
  const lignes = creneauxSaisis.map((saisi, index) => ({ index, creneau: normaliser(saisi) }))
  const exploitables = lignes.filter(
    (ligne): ligne is { index: number; creneau: CreneauHoraire } => ligne.creneau !== null
  )

  // (a) Chevauchements internes au cours en cours de saisie.
...
/** « Lundi 11:00–12:00 chevauche le cours « Groupe Hifz » ». */
export function messageConflit(conflit: ConflitCreneau): string {
  ...
  return conflit.type === 'interne'
    ...
    : `${plage} chevauche le cours « ${conflit.coursLibelle} ».`
}
