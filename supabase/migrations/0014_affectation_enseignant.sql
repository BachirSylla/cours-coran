-- =============================================================================
-- 0014_affectation_enseignant.sql — le responsable choisit qui assure le cours
--
-- Jusqu'ici `enregistrer_cours` posait `enseignant_id = auth.uid()` à la
-- création et n'y touchait plus jamais : affecter un cours à quelqu'un d'autre
-- demandait un UPDATE à la main. La fonction accepte désormais un
-- `enseignant_id` dans sa charge utile.
--
-- **Aucune policy ne change, et c'est délibéré.**
--
--   * Qui a le droit d'affecter ? `cours_insert_responsable` et
--     `cours_update_responsable` (0012) exigent déjà `est_responsable()`. Un
--     enseignant ne peut écrire dans `cours` sous aucun prétexte, donc pas
--     davantage cette colonne-ci.
--   * Peut-on pointer hors du centre ? Non : la clé étrangère COMPOSITE
--     `cours_enseignant_du_centre_fkey (enseignant_id, centre_id)` →
--     `membre (user_id, centre_id)` le refuse structurellement. Un `with check`
--     qui relirait `membre` n'ajouterait aucune garantie — la FK est vérifiée
--     inconditionnellement, y compris par un client qui contournerait
--     PostgREST — et coûterait une sous-requête par écriture de cours.
--
-- `null` = « non fourni » et non « désaffecter » : un client qui n'envoie pas le
-- champ conserve l'affectation en place. C'est ce qui rend la migration sûre
-- vis-à-vis d'une version du front qui ne connaîtrait pas encore le sélecteur.
--
...
begin
  ...
  if p_cours_id is null then
    -- `centre_id` vient du défaut de la table. `enseignant_id` prend la valeur
    -- fournie, ou le créateur à défaut : c'est le comportement d'avant, et il
    -- reste celui de l'enseignant seul, qui n'a personne à qui déléguer.
    ...
    from jsonb_to_record(p_cours) as c(
      ...
      lien_meet text, prix_mensuel numeric, devise text, statut text, enseignant_id uuid
    )
    ...
    set libelle       = c.libelle,
        ...
        -- comme une désaffectation : un ancien client effacerait l'affectation
        -- de tous les cours qu'il enregistre.
   ...
   *
   * Il lit `v_cours.enseignant_id`, c'est-à-dire l'affectation qui vient d'être
   * écrite : réaffecter un cours re-scope donc le contrôle dans la foulée. En
   * revanche il ne porte que sur les créneaux de CE cours — réaffecter ne
   * revalide pas rétroactivement le reste du planning.
   *
   * Bornes STRICTES et aucune marge : deux créneaux adjacents ne se chevauchent
   * pas. `is not distinct from` et non `=` : deux cours SANS enseignant affecté
   * forment un groupe qui se contrôle contre lui-même ; avec `=` la comparaison
   * vaudrait NULL et l'on cesserait silencieusement de contrôler quoi que ce soit.
   */
...
comment on function public.enregistrer_cours(jsonb, jsonb, uuid) is
  'Enregistre un cours, son affectation à un enseignant et ses créneaux dans une seule transaction, et refuse tout chevauchement avec un autre cours du MÊME ENSEIGNANT (CLAUDE.md §5.1).';
